...
func getList(ctx context.Context, db *sql.DB) (videos []TagVideo, err error) {
	...
	*/
	rows, err := db.QueryContext(ctx, `SELECT tags FROM videos WHERE tags LIKE '%best%'`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		var tv TagVideo
		if err = rows.Scan(&tv.Tags); err != nil {
			return nil, err
		}
...
func main() {
	...
	db, err := sql.Open("sqlite", "sprint3/database/update/newvideo.db")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
